...
function getRoleMembers(bytes32 role) public payable { 
    require(msg.data.length - 4 >= 32);
    v0 = new address[](_getRoleMemberCount[role].length);
    v1 = v2 = v0.data;
    if (_getRoleMemberCount[role].length) {
        v3 = v4 = _getRoleMemberCount[role].data;
        while (v2 + (_getRoleMemberCount[role].length << 5) > v1) {
            MEM[v1] = STORAGE[v3];
            v1 += 32;
            v3 += 1;
        }
