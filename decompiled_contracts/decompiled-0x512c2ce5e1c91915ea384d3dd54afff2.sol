...
function 0x17405f67(address varg0, address[] varg1, address[] varg2) public payable { 
    ...
    require(!((v2 + ((varg1.length << 5) + 32) > uint64.max) | (v2 + ((varg1.length << 5) + 32) < v2)), Panic(65)); // failed memory allocation (too much memory)
    v3 = v4 = v2.data;
    ...
    v9 = new address[](varg2.length);
    require(!((v9 + ((varg2.length << 5) + 32) > uint64.max) | (v9 + ((varg2.length << 5) + 32) < v9)), Panic(65)); // failed memory allocation (too much memory)
    v10 = v11 = v9.data;
    require(v8 + (varg2.length << 5) <= 4 + (msg.data.length - 4));
    v12 = v13 = 0;
    while (1) {
        if (v12 >= varg2.length) {
            0x4c(v9, v2, varg0);
